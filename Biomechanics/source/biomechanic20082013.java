...
public class biomechanic20082013 extends PApplet {
...
particleSystem particleAttractor;
public void setup(){
  //size(1024,768,P3D);
  size(800,600,P3D);
  //size(640, 480);
...
//  noStroke();
  // color the boxes
//  fill(#FF8800);
  particleAttractor = new particleSystem();
  noCursor();
}
public void reset(){
...
public void keyPressed(){
  if(key=='r'){reset();}// reinitializes the rag doll
  if(key=='h'){drawHeat=!drawHeat;}
  if(key=='g'){
    gripHead();
  }
  if(key==' '){pushPull=true;}
}
public void gripHead(){
  mouseGripper = new mouseJoint(cubes[0],new PVector(0,-cubes[0].size.y/2,0));
}
public void keyReleased(){
  if(key==' '){pushPull=false;}
}
class cube{
...
// // alternate code to allow for a hollow box
  ...
  public PVector[] stickPoints(){
    // Provides an array of points
    // with which to test a force field.
    // Count the number of points in each direction.
    int a = ceil(size.x/particleSampling);
    int b = ceil(size.y/particleSampling);
    int c = ceil(size.z/particleSampling);
    // initialize an array to store the points in
    PVector[] tp = new PVector[
//      (a+1)*(b+1)*(c+1)];
      (a+1)*(b+1)*(c+1)-(a-1)*(b-1)*(c-1)];
...
class mouseJoint{
  ...
  public void update(){
    if(active){
      ...
      PVector mouse = new PVector(invertedPositionImageX,invertedPositionImageY);
      PVector pmouse = new PVector(invertedPositionImageX,invertedPositionImageY);
      ...
      float F = dx.mag()*0.05f;// spring force of the mouse
      dx.normalize();// make dx a unit vector
      float D = dx.dot(PVector.sub(
...
public PVector[] stickFigure(){
  PVector[] pointCloud = new PVector[0];
  for(int i=0;i<cubes.length;i++){
    pointCloud = (PVector[]) concat(pointCloud,cubes[i].stickPoints());
  }
  return pointCloud;
}
...
class particleSystem{
  ...
  int alphaStep = 32;
  particleSystem(){
    PVector[] pointCloud = stickFigure();
    println(pointCloud.length);
    location = new PVector[pointCloud.length];
    velocity = new PVector[pointCloud.length];
    for(int i=0;i<location.length;i++){
      float rad = sqrt(random(0,1))*256;
      float phi = random(0,TWO_PI);
      location[i] = new PVector(
      width/2+rad*cos(phi),height/2+rad*sin(phi));
...
//    float radialBoundry = mag(width/2,height/2);
    for(int i=0;i<location.length;i++){
      PVector force = PVector.sub(pointCloud[i],location[i]);
      if(force.mag()>16){
        float fMag = 0;
        fMag = gravity/force.mag();
        force.normalize();
        force.mult(fMag);
        if(pushPull){force.mult(-1);}
        velocity[i].add(force);
      }
      velocity[i].mult(1.0f-damping);
      location[i].add(velocity[i]);
//      PVector radial = PVector.sub(location[i],middle);
//      if(radial.mag()>radialBoundry){
//        radial.normalize();
//        radial.mult(2*radialBoundry);
//        location[i].sub(radial);
//      }
      set(PApplet.parseInt(location[i].x),PApplet.parseInt(location[i].y),color(constrain(
      brightness(get(PApplet.parseInt(location[i].x),PApplet.parseInt(location[i].y)))
      +alphaStep,0,255)));
    }
    if(drawHeat){
      colorMode(HSB,256);
      ...
      for(int i=0;i<pixels.length;i++){
        if(brightness(pixels[i])>0){
          pixels[i] = color(256*2/3-brightness(pixels[i])*2/3,255,255);
        }
      }
      updatePixels();
    }
...
public void drawGesture()
{
 ...
 // rect(0,0,125,30);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "biomechanic20082013" };
    if (passedArgs != null) {
